...
def normalize(filename,wout=True):
    ...
    if ch==2:
        ...
        d2=0-max(data_dB[:,1])
        diff=max(d1,d2)
    print('Adding '+str(round(diff,3))+' dB...')
...
def slow(filename,p=30,wout=True):
    """
...
def conv_reverb(filename,ir="vocalduo.wav",wet=1.9,wout=True):  
  start=time.time()
  n, data, data_dB,sr,ch=inputwav(filename)
...
def drakify(filename,wet=0.35):
    ...
    slow(filename[0:len(filename)-4]+'.wav',wout=True,p=30)
